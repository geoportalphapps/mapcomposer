...
Ext.define('MyPath.ExportMap', {
	...
	printMap: function(map, title, imageType, mapReadyCallback){
		...
		var layerLabel = map.getLayersByName('label')[0];
		var labelClone = map.getLayersByName('clone')[0];					
		if (layerLabel.features.length>0){
		    var clone
			for (features in layerLabel.features){
				clone = layerLabel.features[features].clone();				
				labelClone.addFeatures([clone]);														
				console.log(clone);									
			}				
		} 
		//  		
		...
		// overlays		
 		for(layer in printConfig.overlays){
			...
			if (!(printConfig.overlays[layer].renderer instanceof OpenLayers.Renderer.Canvas)) {
				console.log('overlay return', printConfig.overlays[layer].renderer);				
							...
							handler: function(){
								...
								me.printMap(me.map, txtTitle.getValue(), cmbType.getValue(), function(event){
									...
									*/
									...
									console.log(me);
									me.map.getLayersByName('clone')[0].removeAllFeatures()
